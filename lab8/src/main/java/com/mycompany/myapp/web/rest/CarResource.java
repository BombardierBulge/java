...
public class CarResource {
     ...
     */
    @PostMapping("")
    public Mono<ResponseEntity<Car>> createCar(@Valid @RequestBody Car car) throws URISyntaxException {
        LOG.debug("REST request to save Car : {}", car);
        if (car.getId() != null) {
            throw new BadRequestAlertException("A new car cannot already have an ID", ENTITY_NAME, "idexists");
        }
        return carRepository.save(car).map(result -> {
            try {
     ...
     */
    @PutMapping("/{id}")
    public Mono<ResponseEntity<Car>> updateCar(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Car car)
    ...
    public Mono<ResponseEntity<Car>> partialUpdateCar(
        ...
        return carRepository.existsById(id).flatMap(exists -> {
            ...
            Mono<Car> result = carRepository
                .findById(car.getId())
                .map(existingCar -> {
                    updateIfPresent(existingCar::setBrand, car.getBrand());
                    updateIfPresent(existingCar::setModel, car.getModel());
                    updateIfPresent(existingCar::setPricePerHour, car.getPricePerHour());
    ...
    public Mono<ResponseEntity<List<Car>>> getAllCars(
        @org.springdoc.core.annotations.ParameterObject Pageable pageable,
        ServerHttpRequest request,
        @RequestParam(name = "filter", required = false) String filter
    ) {
        if ("driver-is-null".equals(filter)) {
            LOG.debug("REST request to get all Cars where driver is null");
            return carRepository.findAllWhereDriverIsNull().collectList().map(ResponseEntity::ok);
        }
        LOG.debug("REST request to get a page of Cars");
        return carRepository
            .count()
            .zipWith(carRepository.findAllBy(pageable).collectList())
