...
import { sampleWithRequiredData } from '../driver.test-samples';
...
describe('Driver Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 27475 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.drivers()[0]).toEqual(expect.objectContaining({ id: 27475 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 27475 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.drivers()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 27475 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to driverService', () => {
      ...
      vitest.spyOn(service, 'getDriverIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDriverIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.driversParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
