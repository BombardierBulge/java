...
import { sampleWithRequiredData } from '../rental-office.test-samples';
import { RentalOfficeService } from '../service/rental-office.service';

import { RentalOffice } from './rental-office';

vitest.useFakeTimers();

describe('RentalOffice Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: RentalOffice;
  let fixture: ComponentFixture<RentalOffice>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(RentalOffice);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 3069 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.rentalOffices()[0]).toEqual(expect.objectContaining({ id: 3069 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 3069 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.rentalOffices()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 3069 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to rentalOfficeService', () => {
      ...
      vitest.spyOn(service, 'getRentalOfficeIdentifier');
      const id = comp.trackId(entity);
      expect(service.getRentalOfficeIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.rentalOfficesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
